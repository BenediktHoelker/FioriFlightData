sap.ui.define([
	...
	return BaseObject.extend("fdmd.model.GroupSortState", {
		 ...
		 * @alias fdmd.model.GroupSortState
		 */
		...
		/**
		 * Sorts by fldate, or by SEATSMAX
		 *
		...
		/**
		 * Groups by SEATSMAX, or resets the grouping for the key "None"
		 *
		 * @param {string} sKey - the key of the field used for grouping
		 * @returns {sap.ui.model.Sorter[]} an array of sorters
		 */
		group: function (sKey) {
			...
			if (sKey === "SEATSMAX") {
				...
				this._oViewModel.setProperty("/sortBy", "SEATSMAX");

				aSorters.push(
					new Sorter("SEATSMAX", false,
						this._fnGroupFunction.bind(this))
				);
			} else if (sKey === "None") {
				// select the default sorting again
				this._oViewModel.setProperty("/sortBy", "fldate");
			}
