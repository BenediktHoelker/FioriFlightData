			...
			onTheMasterPage : {
				...
				actions : {
							...
							success : function (aEntityData) {
								this.waitFor({
									...
									success : function (oList) {
										var sCurrentId,
											aItemsNotInTheList = aEntityData.filter(function (oObject) {
												return !oList.getItems().some(function (oListItem) {
													return oListItem.getBindingContext().getProperty("fldate") === oObject.fldate;
												});
											});

										if (!aItemsNotInTheList.length) {
											// Not enough items all of them are displayed so we take the last one
											sCurrentId = aEntityData[aEntityData.length - 1].fldate;
										} else {
											sCurrentId = aItemsNotInTheList[0].fldate;
										}
										...
										oCurrentItem.bindingPath = "/" + oList.getModel().createKey("FlightCollection", {
											fldate : sCurrentId
										});
					...
					iRememberTheListItem : function (oListItem) {
						...
						this.getContext().currentItem = {
							...
							id: oBindingContext.getProperty("fldate"),
							title: oBindingContext.getProperty("fldate")
						};
					}
				},
				...
				assertions : {
					...
					theListShouldBeSortedAscendingOnUnitNumber : function () {
						return this.theListShouldBeSortedAscendingOnField("SEATSMAX");
					},

					theListShouldBeSortedAscendingOnName : function () {
						return this.theListShouldBeSortedAscendingOnField("fldate");
					},
					...
					theListShouldHaveAllEntries : function () {
						...
						// retrieve all FlightCollection to be able to check for the total amount
