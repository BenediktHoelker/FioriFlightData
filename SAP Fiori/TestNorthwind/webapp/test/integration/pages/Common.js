sap.ui.define([
		...
		function getFrameUrl (sHash, sUrlParameters) {
			var sUrl = jQuery.sap.getResourcePath("tn/app", ".html");
			sHash = sHash || "";
			sUrlParameters = sUrlParameters ? "?" + sUrlParameters : "";

			if (sHash) {
				sHash = "#TestNorthwind-display&/" + (sHash.indexOf("/") === 0 ? sHash.substring(1) : sHash);
			} else {
				sHash = "#TestNorthwind-display";
			}

			return sUrl + sUrlParameters + sHash;
		}

		return Opa5.extend("tn.test.integration.pages.Common", {
			...
			getMockServer : function () {
				return new Promise(function (success) {
					Opa5.getWindow().sap.ui.require(["tn/localService/mockserver"], function (mockserver) {
